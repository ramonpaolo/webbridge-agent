    ...
    try:
        ...
        try:
            agent_ws = await asyncio.wait_for(
                websockets.connect(AGENT_WS_URL),
                timeout=10
            )
            ...
            await websocket.send_json({"type": "error", "error": "Agent connection timeout"})
            await websocket.close(5002)
            return
        except Exception as e:
            logger.error("Failed to connect to agent: {}", e)
            await websocket.send_json({"type": "error", "error": f"Agent connection failed: {e}"})
